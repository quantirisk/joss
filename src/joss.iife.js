const JOSS = (function() {
  ...
  if (Function.prototype.name === undefined) {
    Object.defineProperty(Function.prototype, "name", {
      get: function() {
        return this.toString().match(/^\s*function(?:\s|\s*\/\*.*\*\/\s*)+([^\(\s\/]*)\s*/)[1];
      }
    });
  }
  Uint8Array.of = Uint8Array.of || function() {
    const result = new this(arguments.length);
    result.set(Array.prototype.slice.call(arguments));
    return result;
  };
  Uint8Array.from = Uint8Array.from || function() {
    const result = new this(arguments[0].length);
    result.set(arguments[0]);
    return result;
  };
  Uint8Array.prototype.forEach = Uint8Array.prototype.forEach || Array.prototype.forEach;
  return {
